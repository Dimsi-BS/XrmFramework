...
{
    /// <summary>
    /// Incremental source generator that materialises an <c>Items</c> collection
    /// on every class decorated with <c>[XrmFramework.EnumGenerationAttribute]</c>.
    ///
    /// Detection happens at the syntax level (cheap walk over <see cref="AttributeSyntax"/>
    /// nodes) and is then resolved through the semantic model so that we only
    /// generate code for classes annotated with the *real* attribute from
    /// <c>XrmFramework</c>.
    /// </summary>
    [Generator]
    public class EnumGenerator : IIncrementalGenerator
    {
        private const string AttributeShortName = "EnumGeneration";
        private const string AttributeFullName = AttributeShortName + "Attribute";

        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            var enumTypes = context.SyntaxProvider
                                   .CreateSyntaxProvider(CouldBeEnumeration, GetEnumTypeOrNull)
        ...
        private static readonly DiagnosticDescriptor EnumGeneratorFailure = new(
            id: "XRM1002",
            title: "EnumGenerator failure",
...
")}";
        ...
        private static bool CouldBeEnumeration(SyntaxNode syntaxNode, CancellationToken cancellationToken)
        {
            ...
            return name is AttributeShortName or AttributeFullName;
        }
        ...
        private static ITypeSymbol? GetEnumTypeOrNull(GeneratorSyntaxContext context, CancellationToken cancellationToken)
        {
            ...
            // attribute.Parent.Parent is the declaration the attribute is applied to.
            if (attributeSyntax.Parent?.Parent is not ClassDeclarationSyntax classDeclaration)
            {
                return null;
            }

            var type = context.SemanticModel.GetDeclaredSymbol(classDeclaration, cancellationToken) as ITypeSymbol;
        ...
        private static bool IsEnumeration(ISymbol type)
            => type.GetAttributes()
                   .Any(a => a.AttributeClass?.Name == AttributeFullName &&
