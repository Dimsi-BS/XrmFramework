...
public class CrmComponentConverter : ICrmComponentConverter
{
	...
	public CrmComponentConverter(ISolutionContext context, ICrmMapper mapper)
	{
		_context = context;
		_mapper = mapper;
	}
	...
	public Entity ToRegisterComponent(ICrmComponent component)
	{
		switch (component)
		{
			case AssemblyInfo assembly:
				return _mapper.ToRemote(assembly);

			case IAssemblyContext assembly:
				return _mapper.ToRemote(assembly.AssemblyInfo);

			case CustomApi customApi:
				return _mapper.ToRemote(customApi);

			case CustomApiRequestParameter request:
				return _mapper.ToRemote(request);

			case CustomApiResponseProperty response:
				return _mapper.ToRemote(response);
			...
			case PluginPackage package:
				return _mapper.ToRemote(package);
...
#pragma warning restore 0612
			Rank = step.Order,
			SdkMessageId = _context.GetMessage(step.Message),
			SdkMessageFilterId = _context.GetMessageFilter(step.Message, step.EntityName),
