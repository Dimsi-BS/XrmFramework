...
public class AssemblyDiffFactory
{
	...
	public AssemblyDiffFactory(ICrmComponentComparer comparer, ICrmMapper mapper)
	{
		_comparer = comparer;
		_mapper = mapper;
	}
	...
	public IAssemblyContext ComputeDiffPatch(IAssemblyContext from, IAssemblyContext target)
	{
		...
		var fromCopy = _mapper.Clone(from);
		...
		var targetCopy = _mapper.Clone(target);
