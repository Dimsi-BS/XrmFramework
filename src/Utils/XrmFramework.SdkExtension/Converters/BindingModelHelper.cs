...
namespace Model
{
    public static class BindingModelHelper
    {
        ...
        {
            T returnBinding = default(T);
            if (!String.IsNullOrEmpty(typeName) && !idEntity.Equals(Guid.Empty))
            {
                Dictionary<Guid, object> CachedEntities = null;

                cache.TryGetValue(typeName, out CachedEntities);

                if (CachedEntities != null)
                {
                    object cachedBinding = null;
                    CachedEntities.TryGetValue(idEntity, out cachedBinding);
        ...
        {
            bool bRet = false;
            object res = FindBinding<object>(cache, typeName, idEntity);
            if (res != null)
            {
                bRet = true;
            }
