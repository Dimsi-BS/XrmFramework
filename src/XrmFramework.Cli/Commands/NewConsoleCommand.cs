...
/// <summary>
/// <c>xrmframework new console</c> command: adds a console app project to an existing solution,
/// replacing the <c>xrmConsoleProject</c> dotnet-new template. Logic in
/// <see cref="SimpleProjectScaffolder.Create(string, string, string, string)"/>.
/// </summary>
public sealed class NewConsoleCommand : Command<NewConsoleCommand.Settings>
{
    public sealed class Settings : CommandSettings
    {
        [CommandArgument(0, "<NAME>")]
        [System.ComponentModel.Description("Name of the console project.")]
    ...
    {
        var templateRoot = Path.Combine(AppContext.BaseDirectory, "Scaffolding", "ConsoleApp");
        var solutionRoot = settings.SolutionDirectory ?? Directory.GetCurrentDirectory();

        return SimpleProjectScaffolder.Create(templateRoot, settings.Name!, solutionRoot, "console");
    }
}
