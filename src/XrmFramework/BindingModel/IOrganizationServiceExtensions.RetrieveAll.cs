...
public static class IOrganizationServiceExtensions_RetrieveAll
{
    ...
    extension(IOrganizationServiceAsync service)
    {
        ...
        public async Task<IList<T>> RetrieveAllAsync<T>(QueryExpression query, bool cleanLinks = false)
    ...
    {
        public async Task<IList<T>> RetrieveAllAsync<T>(CancellationToken cancellationToken) where T : IBindingModel
        {
            var query = BindingModelHelper.GetRetrieveAllQuery<T>();
            return (await service.RetrieveAllAsync(query, cancellationToken)).ToBindingModel<T>().ToList();
        }

        public async Task<IList<T>> RetrieveAllAsync<T>(QueryExpression query, CancellationToken cancellationToken,
            bool cleanLinks = false) where T : IBindingModel
        {
            return (await service.RetrieveAllAsync(query, cancellationToken, cleanLinks)).ToBindingModel<T>().ToList();
        }

        public async Task<IList<Entity>> RetrieveAllAsync(QueryExpression query, CancellationToken cancellationToken,
            bool cleanLinks = false)
