...
using System.Linq;
using DefinitionManager.Definitions;
using Deploy;
...
internal class DataAccessManager : AsyncManager
{
	...
	public static DataAccessManager Instance { get; } = new();
	...
	private object DoConnect(object arg)
	{
		...
		PublisherPrefixes.AddRange(
			solutions.Select(
					s => s.GetAttributeValue<AliasedValue>($"{PublisherDefinition.EntityName}.{PublisherDefinition.Columns.CustomizationPrefix}").Value as string)
.Where(s => !string.IsNullOrWhiteSpace(s)).Distinct());
	...
	private object DoRetrieveEntities(object arg)
	{
		...
		var publisherPrefixes = _service.RetrieveMultiple(queryPublishers).Entities
.Select(e => e.GetAttributeValue<string>(PublisherDefinition.Columns.CustomizationPrefix)).ToList();
		...
		foreach (var entity in entitiesMetadata)
		{
			...
			if (entity.Keys != null && entity.Keys.Any())
			{
				...
				{
					keyDefinition.Attributes.Add(new AttributeDefinition
					{
						LogicalName = key.LogicalName.Trim('"'), Name = key.DisplayName.UserLocalizedLabel.Label.FormatText(), Value = key.LogicalName.Trim('"'),
						Type = "String"
					});
			...
			if (entity.ManyToManyRelationships.Any())
			{
				...
				foreach (var relationship in entity.ManyToManyRelationships)
				{
					classDefinition.Attributes.Add(new RelationshipAttributeDefinition
					{
						...
						TargetEntityName = relationship.Entity1LogicalName == entityDefinition.LogicalName
							? relationship.Entity2LogicalName
							: relationship.Entity1LogicalName,
						LookupFieldName = relationship.Entity1LogicalName == entityDefinition.LogicalName
					...
					newEntity.ManyToManyRelationships.Add(new Relation
					{
						...
						EntityName = relationship.Entity1LogicalName == entityDefinition.LogicalName
							? relationship.Entity2LogicalName
							: relationship.Entity1LogicalName,
						NavigationPropertyName = relationship.IntersectEntityName,
						LookupFieldName = relationship.Entity1LogicalName == entityDefinition.LogicalName
			...
			foreach (var attributeMetadata in entity.Attributes.OrderBy(a => a.LogicalName))
			{
				...
				if (attributeMetadata.AttributeType.Value == AttributeTypeCode.Picklist || attributeMetadata.AttributeType.Value == AttributeTypeCode.State ||
				    attributeMetadata.AttributeType.Value == AttributeTypeCode.Status || (attributeMetadata.AttributeType.Value == AttributeTypeCode.Virtual &&
				...
				var attributeDefinition = new AttributeDefinition
				{
					...
					Name = name,
					DisplayName = attributeMetadata.DisplayName.UserLocalizedLabel == null
				...
				var attribute = new Column
				{
					LogicalName = attributeMetadata.LogicalName,
					Name = name,
					Type = (XrmFramework.AttributeTypeCode) (int) (attributeMetadata.AttributeType.Value == AttributeTypeCode.Virtual &&
					                                               attributeMetadata is MultiSelectPicklistAttributeMetadata
					...
					PrimaryType = attributeMetadata.LogicalName == entity.PrimaryIdAttribute ? PrimaryType.Id :
						attributeMetadata.LogicalName == entity.PrimaryNameAttribute         ? PrimaryType.Name :
				...
				if (attributeDefinition.IsPrimaryIdAttribute || attributeDefinition.IsPrimaryNameAttribute || attributeDefinition.IsPrimaryImageAttribute ||
				    attributeDefinition.KeyNames.Any())
				{
	...
	private object DoRetrieveAttributes(object item)
	{
		...
		foreach (var attributeMetadata in response.EntityMetadata.Attributes.OrderBy(a => a.LogicalName))
		{
			if (!attributeMetadata.IsValidForCreate.Value && !attributeMetadata.IsValidForRead.Value && !attributeMetadata.IsValidForUpdate.Value)
			{
				continue;
			...
			if (attributeMetadata.IsLogical.Value || attributeMetadata.AttributeType.Value == AttributeTypeCode.EntityName ||
			...
			if (attributeMetadata.AttributeType.Value == AttributeTypeCode.Picklist || attributeMetadata.AttributeType.Value == AttributeTypeCode.State ||
			    attributeMetadata.AttributeType.Value == AttributeTypeCode.Status || (attributeMetadata.AttributeType == AttributeTypeCode.Virtual &&
			                                                                          attributeMetadata is MultiSelectPicklistAttributeMetadata))
			{
				var meta = ((EnumAttributeMetadata) attributeMetadata).OptionSet;
	...
	{
		// Use the prefix that has the most first letters in common with the name
		var bestPrefix = PublisherPrefixes
		...
		{
			name = name.Substring(bestPrefix.Length + 1);
		}
