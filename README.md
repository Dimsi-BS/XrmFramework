...
`XrmFramework.TypeScript` brings the same "typed, no boilerplate" philosophy to client-side scripting: a `Webresources` project generated by `xrmframework new solution` builds every `.ts` file under `scripts/` into its own web resource with webpack, gives form/ribbon scripts a typed `UtilsApi<TTable>` helper, and regenerates the `*Definition` types straight from your `.table` files so field names, picklist values and control types are checked at compile time.

-> **[XrmFramework TypeScript](docs/XrmFrameworkTypeScript.md)**
...
| [TypeScript](docs/XrmFrameworkTypeScript.md) | Client-side scripting: typed form/ribbon scripts, the `UtilsApi` helper, and the webpack build pipeline. |
...
| [XrmFramework.TypeScript](https://www.nuget.org/packages/XrmFramework.TypeScript) | TypeScript tooling for web resources — see **[docs](docs/XrmFrameworkTypeScript.md)** | ![NuGet](https://img.shields.io/nuget/v/XrmFramework.TypeScript?logo=nuget&label=) | ![Downloads](https://img.shields.io/nuget/dt/XrmFramework.TypeScript?color=success&label=) |
